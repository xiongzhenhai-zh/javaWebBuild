...
public class FollowConfig {
	...
	private static List<Model> initModels(Project project) {
		...
		// 模型列表
		Model followLibrary  = new Model(getId(), "FollowLibrary", "随访库", "uuid", project);
		followLibrary.addAttribute(new ModelAttribute(getId(),"name", "库名称", "String"));
		followLibrary.addAttribute(new ModelAttribute(getId(),"masterType","所属人类型","String"));
		followLibrary.addAttribute(new ModelAttribute(getId(),"masterId","所属人Id","Integer"));
		followLibrary.addAttribute(new ModelAttribute(getId(),"createTime","创建时间","Date"));
		
		Model followQuestionnaire  = new Model(getId(), "FollowQuestionnaire", "随访问卷版本", "uuid", project);
		followQuestionnaire.addAttribute(new ModelAttribute(getId(),"type", "问卷类型", "String"));
		followQuestionnaire.addAttribute(new ModelAttribute(getId(),"status","状态","Integer"));
		followQuestionnaire.addAttribute(new ModelAttribute(getId(),"name","问卷名","String"));
		followQuestionnaire.addAttribute(new ModelAttribute(getId(),"createTime","创建时间","Date"));
		
		Model followQuestionnaireVersion  = new Model(getId(), "FollowQuestionnaireVersion", "随访问卷版本", "uuid", project);
		followQuestionnaireVersion.addAttribute(new ModelAttribute(getId(),"note", "备注", "String"));
		followQuestionnaireVersion.addAttribute(new ModelAttribute(getId(),"status","状态","Integer"));
		followQuestionnaireVersion.addAttribute(new ModelAttribute(getId(),"json","版本数据","String"));
		followQuestionnaireVersion.addAttribute(new ModelAttribute(getId(),"createTime","创建时间","Date"));
		
		
		// 关系列表
		Relation followLibraryParent = new Relation(getId(), "parent", "ManyToOne", followLibrary);
		Relation followLibraryChildren = new Relation(getId(), "children", "OneToMany", followLibrary);
		
		Relation followQuestionnaireFollowLibrary = new Relation(getId(), "followLibrary", "ManyToOne", followQuestionnaire);
		Relation followLibraryFollowQuestionnaire = new Relation(getId(), "followQuestionnaires", "OneToMany", followLibrary);
		
		Relation followQuestionnaireVersionFollowQuestionnaire = new Relation(getId(), "followQuestionnaire", "ManyToOne", followQuestionnaireVersion);
		Relation followQuestionnaireFollowQuestionnaireVersion = new Relation(getId(), "followQuestionnaireVersions", "OneToMany", followQuestionnaire);
		
		// 关系设置
		followLibraryParent.setRelation(followLibraryChildren);
		followLibraryChildren.setRelation(followLibraryParent);
		
		followLibraryFollowQuestionnaire.setRelation(followQuestionnaireFollowLibrary);
		followQuestionnaireFollowLibrary.setRelation(followLibraryFollowQuestionnaire);
		
		followQuestionnaireFollowQuestionnaireVersion.setRelation(followQuestionnaireVersionFollowQuestionnaire);
		followQuestionnaireVersionFollowQuestionnaire.setRelation(followQuestionnaireFollowQuestionnaireVersion);
		
		// 添加关系
		followLibrary.addRelation(followLibraryChildren);
		followLibrary.addRelation(followLibraryParent);
		followLibrary.addRelation(followLibraryFollowQuestionnaire);
		
		followQuestionnaire.addRelation(followQuestionnaireFollowLibrary);
		followQuestionnaire.addRelation(followQuestionnaireFollowQuestionnaireVersion);
		
		followQuestionnaireVersion.addRelation(followQuestionnaireVersionFollowQuestionnaire);
		
		// 添加模型
		models.add(followLibrary);
		models.add(followQuestionnaire);
		models.add(followQuestionnaireVersion);
		return models;
	}
	...
	public static Project getProject() {
		Project project = new Project("123456", "FollowConfig", "BBcare管理平台", new Frame(getId(), "springmvc", "SPRINGMVC"),
		...
		project.setDataSource(new DataSource(
						...
						"mysql",
						"mysql",
						"mysql://localhost:3306/test_followScheme_server?createDatabaseIfNotExist=true&amp;useUnicode=true&amp;characterEncoding=utf8&amp;zeroDateTimeBehavior=convertToNull&amp;transformedBitIsBoolean=true",
						"root", "123456"));
		return project;
	}
}

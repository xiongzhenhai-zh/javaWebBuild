...
public class CareJava {
	...
	private static List<Model> initModels(Project project) {
		...
		// 模型列表
		Model user = new Model(getId(), "User", "用户表", "uuid", project);
		user.addAttribute(new ModelAttribute(getId(), "name", "昵称", "String"));
		user.addAttribute(new ModelAttribute(getId(), "createTime", "注册时间", "Date"));
		user.addAttribute(new ModelAttribute(getId(), "userName", "用户名", "String"));
		user.addAttribute(new ModelAttribute(getId(), "password", "密码", "String"));
		user.addAttribute(new ModelAttribute(getId(), "status", "状态", "String"));
		user.addAttribute(new ModelAttribute(getId(), "type", "类型", "String"));
		
		Model article = new Model(getId(), "Article", "文章", "uuid", project);
		article.addAttribute(new ModelAttribute(getId(), "title", "标题", "String"));
		article.addAttribute(new ModelAttribute(getId(), "createTime", "注册时间", "Date"));
		article.addAttribute(new ModelAttribute(getId(), "keyWord", "关键字", "String"));
		article.addAttribute(new ModelAttribute(getId(), "content", "内容", "String"));
		article.addAttribute(new ModelAttribute(getId(), "status", "状态", "String"));
		article.addAttribute(new ModelAttribute(getId(), "type", "类型", "String"));
		
		// 关系列表
	
		Relation articleUser = new Relation(getId(), "user", "ManyToOne",  article);
		Relation userArticle = new Relation(getId(), "articles", "OneToMany",  user);
		
		// 关系设置
		articleUser.setRelation(userArticle);
		userArticle.setRelation(articleUser);
		
		// 添加关系
		user.addRelation(userArticle);
		article.addRelation(articleUser);
	...
	public static Project getProject() {
		Project project = new Project("123456", "CareJava", "careJava管理平台", new Frame(getId(), "springmvc", "SPRINGMVC"),
				new Frame(getId(), "smartadmin", "SmartAdmin"), new Frame(getId(), "jquerymobile", "jquerymobile"));
		project.setPkgName("com.careJava.code");
		...
		project.setDataSource(new DataSource(
						...
						"mysql",
						"mysql",
						"mysql://localhost:3306/carejava_server?createDatabaseIfNotExist=true&amp;useUnicode=true&amp;characterEncoding=utf-8&amp;zeroDateTimeBehavior=convertToNull&amp;transformedBitIsBoolean=true",
						"root", "123456"));
		return project;
	}
}

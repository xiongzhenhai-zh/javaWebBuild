...
public class PrivateDoctor1 {
	...
	private static List<Model> initModels(Project project) {
		...
		expertorConsult.addAttribute(new ModelAttribute(getId(),"timeSolt","时间段","Integer"));
		...
		babyProduct.addAttribute(new ModelAttribute(getId(),"type","产品类型","Integer"));
		...
		Model agency=new Model(getId(),"Agency","机构","uuid",project);
		...
		// 关系列表
		Relation agencyProduct=new Relation(getId(),"products","OneToMany",agency);
		Relation productAgency=new Relation(getId(),"agency","ManyToOne",product);
		...
		// 关系设置
		agencyProduct.setRelation(productAgency);
		productAgency.setRelation(agencyProduct);
		...
		// 添加关系
		agency.addRelation(agencyProduct);

		product.addRelation(productAgency);

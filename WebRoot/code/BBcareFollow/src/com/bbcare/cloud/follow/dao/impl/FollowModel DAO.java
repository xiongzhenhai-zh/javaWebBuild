...
@Repository	
public class FollowModel DAO implements IFollowModel DAO {	
	
	@Autowired	
	private HibernateTemplate hibernateTemplate;	
	
	public String save(FollowModel  followModel ) throws Exception {	
		return (String) hibernateTemplate.save(followModel );	
	}
	  
    public void delete(String id) throws Exception {	
		hibernateTemplate.delete(hibernateTemplate.get(FollowModel .class, id));	
	}	
	
   	public void update(FollowModel  followModel ) throws Exception {	
		hibernateTemplate.update(followModel );	
	}
	
    public FollowModel  get(String id) throws Exception {	
		return hibernateTemplate.get(FollowModel .class, id);	
	}
	
    @SuppressWarnings({"unchecked", "rawtypes" })	
	public List<FollowModel > list(final DetachedCriteria dc,final int page,final int rows) throws Exception {	
		return hibernateTemplate.executeFind( new  HibernateCallback(){
			public List<FollowModel > doInHibernate(Session session)  throws  SQLException,
			HibernateException {
				...
				return (List<FollowModel >)criteria.list();
			}
		});	
	}	

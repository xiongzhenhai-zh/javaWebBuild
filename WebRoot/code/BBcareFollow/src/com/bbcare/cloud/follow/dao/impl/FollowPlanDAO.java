...
@Repository	
public class FollowPlanDAO implements IFollowPlanDAO {	
	
	@Autowired	
	private HibernateTemplate hibernateTemplate;	
	
	public String save(FollowPlan followPlan) throws Exception {	
		return (String) hibernateTemplate.save(followPlan);	
	}
	  
    public void delete(String id) throws Exception {	
		hibernateTemplate.delete(hibernateTemplate.get(FollowPlan.class, id));	
	}	
	
   	public void update(FollowPlan followPlan) throws Exception {	
		hibernateTemplate.update(followPlan);	
	}
	
    public FollowPlan get(String id) throws Exception {	
		return hibernateTemplate.get(FollowPlan.class, id);	
	}
	
    @SuppressWarnings({"unchecked", "rawtypes" })	
	public List<FollowPlan> list(final DetachedCriteria dc,final int page,final int rows) throws Exception {	
		return hibernateTemplate.executeFind( new  HibernateCallback(){
			public List<FollowPlan> doInHibernate(Session session)  throws  SQLException,
			HibernateException {
				...
				return (List<FollowPlan>)criteria.list();
			}
		});	
	}	
